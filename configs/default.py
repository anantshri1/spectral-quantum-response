...
class Config:
    ...
    t_end: float = 0.5     # TUNE (Phase 1/2): long enough for off-diagonal coupling,
                           # short enough that forward stays learnable + phase error small.

    # --- ψ0 family: Gaussian wavepackets (TUNE ranges in Phase 2) ---
    psi0_x0_lo: float = 2.0     # center stays away from box edges (L≈6.28) to avoid wrap
    psi0_x0_hi: float = 4.28
    ...
    # --- V family: smooth GRF (TUNE in Phase 2; this is the complexity/OOD dial) ---
    v_length_scale: float = 1.0   # smaller = more high-k structure = harder response
